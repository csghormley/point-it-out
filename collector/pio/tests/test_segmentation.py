"""
Unit tests for survey point segmentation functionality.
...
class TemporalSegmentationTest(TestCase):
...
class SpatialClusteringTest(TestCase):
...
class LinearityCalculationTest(TestCase):
...
class ConvexHullPolygonTest(TestCase):
...
class GeoJSONFileLoadingTest(TestCase):
    """Tests for loading survey points from GeoJSON files"""

    def test_load_valid_geojson(self):
        """Should successfully load points from valid GeoJSON"""
        from pio.management.commands.segment_survey_points import Command
        ...
        geojson_data = {
            ...
            "features": [
                ...
                },
                {
                    ...
                    "geometry": {
                        "type": "Point",
                        "coordinates": [-121.47, 46.16]
                    },
                    "properties": {
                        "id": 2,
                        "responseid": "test",
                        "projectid": 1,
                        "timestamp_add": "2025-11-01T12:01:00+00:00",
                        "radius": 150.0
                    }
                }
            ]
        ...
        try:
            cmd = Command()
            points = cmd._load_points_from_file(temp_path)

            self.assertEqual(len(points), 2)
            self.assertEqual(points[0].x, -121.48)
            self.assertEqual(points[0].y, 46.15)
            self.assertEqual(points[0].radius, 100.0)
            self.assertEqual(points[1].radius, 150.0)
        finally:
            Path(temp_path).unlink()

    def test_load_geojson_skip_non_points(self):
        """Should skip non-point geometries"""
        from pio.management.commands.segment_survey_points import Command
        ...
        geojson_data = {
            ...
            "features": [
                ...
                },
                {
                    ...
                    "geometry": {
                        "type": "LineString",
                        "coordinates": [[-121.48, 46.15], [-121.47, 46.16]]
                    },
                    "properties": {
                        "id": 2,
                        "responseid": "test",
        ...
        try:
            ...
            # Should only load the Point feature, skip LineString
